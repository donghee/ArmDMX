...
Strip* Strip_new(uint16_t length, uint8_t data_pin, uint8_t clock_pin)
{
    ...
    self->frame_count = 0;
    self->cursor = 0;
    self->cursor_speed = 1; // dotflow speed

 	GPIOSetDir(LED_STRIP_PORT, data_pin, OUT);
    GPIOSetDir(LED_STRIP_PORT, clock_pin, OUT);
...
void Strip_shows(Strip** strips)
{
	...
	for (i = 0; i < strips[0]->length; i++) // TODO length is largest length of strips
	{
		rgb_steps(strips[0]->pixels[i], strips[0]->data_pin,
				strips[1]->pixels[i], strips[1]->data_pin,
				strips[2]->pixels[i], strips[2]->data_pin,
				strips[3]->pixels[i], strips[3]->data_pin);
	}
	 LPC_GPIO[LED_STRIP_PORT]->MASKED_ACCESS[ALL_CLOCKPINS_MASK] = 0;
}
